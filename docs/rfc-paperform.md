# RFC: paperform — The Judgment Layer
...
- **paper-doll/v2 only.** Name-anchored vessel matching and token-based element checks need no element ids, so paperform does **not** depend on paper-doll/v3 — unlike paperchain and paperfold, both blocked on the address grammar. paperform drafting and implementation can proceed in parallel with v3.
...
1. **paperform v1 drafting** — this document; proceeds now, independent of paper-doll/v3 (no shared prerequisite — see Dependencies). Pure conformance checking can ship before, alongside, or after the v3/paperfold track.
...
The npm name `paperform` is available as of 2026-07-10 and not yet registered — to be reserved alongside the family's placeholders.
